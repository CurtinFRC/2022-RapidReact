...
Intake::Intake(RobotMap.IntakeSystem &intakeSystem, SmartControllerGroup &contGroup) : _intakeSystem(intakeSystem), _contGroup(contGroup) {}

void Intake::setIntake(double voltage) {
  // _intakeSpeed = voltage;
}

void Intake::_toggleIntake() {
  // _intakeState == IntakeStates::kDeployed ? setState(IntakeStates::kStowed) : setState(IntakeStates::kDeployed);
}
...
void updateIntake(double dt) {
  ...
  //   default: 
  //     std::cout << "in Intake defult state something is wrong" << std::endl;
  //     _state = IntakeStates::STOWED; 
  //   break;
  // }

  // _intakeSystem.intakeSolenoid.Update(dt);
  // _intakeSystem.intake.SetVoltage(_intakeSpeed);
}
